<?php
// Our database class
if(!class_exists('MStoreDatabase')){
	class MStoreDatabase {
		 ...
		 *
		 * @uses MStoreDatabase::__construct()
		 *
		 */	
		function MStoreDatabase() {
		...
		function connect() {
			...
			$db_selected = mysql_select_db('musicstore', $link);

			if (!$db_selected) {
				die('Can\'t use ' . musicstore . ': ' . mysql_error());
			}
		}
		...
		function hash_password($password, $nonce) {
		  $secureHash = hash_hmac('sha512', $password . $nonce, SITE_KEY);//use two salts to hash the password
		  return $secureHash;
		}
		...
		function insert($link, $table, $fields, $values) {
			$fields = implode(", ", $fields); //implode converts the filed array into a string og the format ", "
			$values = implode("', '", $values);
			$sql="INSERT INTO $table (id, $fields) VALUES ('', '$values')";//id is auto increment so we dont pass it explicitly we just menation it
